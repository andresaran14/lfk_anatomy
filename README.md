...
Each result card includes a "View in 3D" button that loads an interactive 3D model of the matching body system (skeletal, muscular, nervous, or joint/ligament), sourced from **[Z-Anatomy](https://www.z-anatomy.com/)**, an open-source anatomy atlas, hosted via **[Sketchfab](https://sketchfab.com/)**.

- Models are licensed **CC BY-SA** — attribution is shown automatically in the app whenever a model is opened, and must be preserved if you redistribute this project.
- Z-Anatomy publishes by whole body system rather than one model per structure, so the viewer shows the full system (e.g. the entire skeleton) — rotate and zoom to find the specific structure you're studying.
