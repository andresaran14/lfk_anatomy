...
// Show the initial "type something to search" state — cards only appear once
// the student actually searches for a structure. The instructions/examples
// panel is only useful before that, so it reappears here and collapses
// everywhere else to keep the page decluttered once results are showing.
function showStartPromptMessage() {
    active3DImageDiv = null;
    instructionsEl.classList.remove('is-collapsed');
    resultsGrid.innerHTML = `
        <div class="no-results">
            <h3>Start typing to find a structure</h3>
            <p>Describe a location, function, or characteristic above, or tap one of the example searches.</p>
        </div>
    `;
}
...
function createResultCard(item) {
    ...
    categorySpan.className = `result-category category-${item.category}`;
