...
// Free, open-source (CC BY-SA) 3D anatomy models by Z-Anatomy, hosted on Sketchfab.
// Z-Anatomy publishes by whole body system rather than one model per structure, so
// each category links to the matching system model — students rotate/zoom to find
// the specific structure within it. Attribution is required by the license and is
// shown in the UI whenever a model is opened.
// Apple Health-style category identity: a vivid gradient badge + simple line
// icon per tissue type, mirroring how Health assigns each metric category its
// own colored icon. Built from primitive shapes (not hand-drawn paths) to
// keep them simple and reliable at small sizes.
const CATEGORY_STYLE = {
    bone: {
        label: 'Bone',
        gradient: ['#d3b48c', '#9c7b4f'],
        iconSVG: `<g transform="rotate(45 12 12)">
            <rect x="7.5" y="10.4" width="9" height="3.2" rx="1.6" fill="white"/>
            <circle cx="7" cy="9" r="2.4" fill="white"/>
            <circle cx="7" cy="15" r="2.4" fill="white"/>
            <circle cx="17" cy="9" r="2.4" fill="white"/>
            <circle cx="17" cy="15" r="2.4" fill="white"/>
        </g>`
    },
    muscle: {
        label: 'Muscle',
        gradient: ['#ff6b5b', '#e0342a'],
        iconSVG: `<rect x="4" y="10.4" width="16" height="3.2" rx="1.6" fill="white"/>
            <rect x="2" y="7" width="4.2" height="10" rx="1.6" fill="white"/>
            <rect x="17.8" y="7" width="4.2" height="10" rx="1.6" fill="white"/>`
    },
    nerve: {
        label: 'Nerve',
        gradient: ['#ffd60a', '#ff9f0a'],
        iconSVG: `<path d="M13 2 L4.5 13.5h5.5l-1.2 8.5 9.2-12.5h-5.7z" fill="white"/>`
    },
    ligament: {
        label: 'Ligament',
        gradient: ['#64d9d0', '#1f9c94'],
        iconSVG: `<g fill="none" stroke="white" stroke-width="2.6" stroke-linecap="round">
            <rect x="2.6" y="7.2" width="10" height="9.6" rx="4.8" transform="rotate(-30 7.6 12)"/>
            <rect x="11.4" y="7.2" width="10" height="9.6" rx="4.8" transform="rotate(-30 16.4 12)"/>
        </g>`
    }
};

function categoryBadgeHTML(category, size = 'md') {
    const style = CATEGORY_STYLE[category];
    if (!style) return '';
    return `<div class="category-badge category-${category} size-${size}">
        <svg viewBox="0 0 24 24" class="category-icon">${style.iconSVG}</svg>
    </div>`;
}

const CATEGORY_3D_MODELS = {
    bone: {
        uid: '657a31ed9704423c8c4e752fb2506a74',
        title: 'Human Skeleton (Highresolution)',
    ...
    ligament: {
        ...
        title: 'Arthrology',
        systemLabel: 'joints & ligaments system',
        author: 'Z-Anatomy',
        url: 'https://sketchfab.com/3d-models/arthrology-a890d801336047d683d56d8bc676e894',
        license: 'CC BY-SA'
    }
};

// Only one embedded 3D viewer (WebGL context) is allowed to be active at a time —
// several running at once is what tends to crash mobile Safari / iPad browsers.
let active3DImageDiv = null;
...
// Initialize the page — cards only appear once the student searches for something
document.addEventListener('DOMContentLoaded', () => {
    showStartPromptMessage();
    // Focus on search input when page loads
    searchInput.focus();

    // Example chips act as one-tap searches
...
// Live search — the dataset is tiny, so results update on every keystroke
// with no debounce, keeping the input path as responsive as possible.
...
function displayResults(items) {
    ...
    active3DImageDiv = null;
    instructionsEl.classList.add('is-collapsed');
...
// Show the initial "browse by system" state — a Health-style grid of the
// four tissue categories, plus the search examples. Cards only appear once
// the student actually searches or taps a category.
function showStartPromptMessage() {
    active3DImageDiv = null;
    instructionsEl.classList.remove('is-collapsed');
    ...
    const tilesHTML = Object.keys(CATEGORY_STYLE).map((category) => {
        const count = counts[category] || 0;
        return `
        <button type="button" class="browse-tile" data-category="${category}">
            ${categoryBadgeHTML(category, 'lg')}
            <span class="browse-tile-label">${CATEGORY_STYLE[category].label}s</span>
            <span class="browse-tile-count">${count} structure${count === 1 ? '' : 's'}</span>
        </button>`;
    }).join('');

    resultsGrid.innerHTML = `
        <div class="browse-section">
            <p class="browse-heading">Browse by system</p>
            <div class="browse-grid">${tilesHTML}</div>
        </div>
    `;

    resultsGrid.querySelectorAll('.browse-tile').forEach((tile) => {
        tile.addEventListener('click', () => {
            browseCategory(tile.dataset.category);
        });
    });
}

// Show every structure in a given category — used by the Browse grid tiles
function browseCategory(category) {
    const items = anatomyData.filter((item) => item.category === category);
...
function createResultCard(item) {
    ...
    card.className = `result-card category-${item.category}`;
    ...
    // "View in 3D" toggle — the WebGL viewer is only created when the student
    // actually asks for it, and only one stays active at a time.
    const model3D = CATEGORY_3D_MODELS[item.category];
    if (model3D) {
        const toggle3DBtn = document.createElement('button');
        toggle3DBtn.type = 'button';
        toggle3DBtn.className = 'view-3d-btn';
        toggle3DBtn.textContent = '🔄 View in 3D';
        ...
        function close3D() {
            ...
            imageDiv.classList.remove('mode-3d');
            img.style.display = '';
            toggle3DBtn.textContent = '🔄 View in 3D';
            is3DActive = false;
            if (active3DImageDiv === imageDiv) {
                active3DImageDiv = null;
            }
        }
        ...
        toggle3DBtn.addEventListener('click', () => {
            ...
            // Shut down any other open 3D viewer first (one WebGL context at a time)
            if (active3DImageDiv && active3DImageDiv._close3D) {
                active3DImageDiv._close3D();
            }

            img.style.display = 'none';
            imageDiv.classList.add('mode-3d');

            iframeEl = document.createElement('iframe');
            iframeEl.className = 'model-3d-frame';
            iframeEl.title = `${model3D.title} 3D model — ${model3D.author}`;
            iframeEl.src = `https://sketchfab.com/models/${model3D.uid}/embed?autostart=1&ui_theme=dark&ui_hint=2&dnt=1&transparent=1`;
            iframeEl.setAttribute('frameborder', '0');
            iframeEl.setAttribute('allow', 'autoplay; fullscreen; xr-spatial-tracking');
            iframeEl.setAttribute('allowfullscreen', '');
            imageDiv.appendChild(iframeEl);

            captionEl = document.createElement('p');
            captionEl.className = 'model-3d-caption';
            captionEl.innerHTML = `Showing the full ${model3D.systemLabel} — rotate/zoom to find "${item.name}". Model: <a href="${model3D.url}" target="_blank" rel="noopener noreferrer">${model3D.title}</a> by ${model3D.author} (Sketchfab${model3D.license ? ', ' + model3D.license : ''})`;
            imageDiv.appendChild(captionEl);

            toggle3DBtn.textContent = '✕ Close 3D view';
            is3DActive = true;
            imageDiv._close3D = close3D;
            active3DImageDiv = imageDiv;
        });

        imageDiv.appendChild(toggle3DBtn);
    }
    ...
    // Header — Health-style: colored icon badge leading the title block
    const headerDiv = document.createElement('div');
    headerDiv.className = 'result-header';
    headerDiv.innerHTML = categoryBadgeHTML(item.category);

    const titlesDiv = document.createElement('div');
    titlesDiv.className = 'result-titles';

    const nameH2 = document.createElement('h2');
    nameH2.className = 'result-name';
    nameH2.textContent = item.name;

    const subcatP = document.createElement('p');
    subcatP.className = 'result-subcategory';
    subcatP.textContent = formatCategory(item.category, item.subcategory);

    titlesDiv.appendChild(nameH2);
    titlesDiv.appendChild(subcatP);
    ...
    // Add key details as a grouped-list, Health-style
    const detailsItems = Object.entries(item.details || {});
    if (detailsItems.length > 0) {
        const detailsList = document.createElement('div');
        detailsList.className = 'detail-rows';
